...
export default function ListTeam() {
  ...
  useEffect(() => {
    const teamData = getStorageData('teamData', allData.find(m => m.module === "Team")?.data || []);
    setTeams(teamData);
  }, []);

  function handleDelete(id: any): void {
    const existingData = getStorageData('teamData', allData.find(m => m.module === "Team")?.data || []);
    const updatedData = existingData.filter((team: any) => team.id !== id);
  ...
  return (
    <div className="m-4">
      <div className="flex flex-row justify-between mb-4">
        <p className="font-bold text-2xl">Teams</p>
          ...
          <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
            {teams.map((team) => (
              <tr key={team.id} className="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors duration-200">
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">{team.name}</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">{team.roles?.join(", ") || ""}</td>
                <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                  ...
                  <button
                    ...
                    className="text-red-600 hover:text-red-900 dark:text-red-400 dark:hover:text-red-300 font-medium ml-2"
                  >
                    Delete
                  </button>
                </td>
